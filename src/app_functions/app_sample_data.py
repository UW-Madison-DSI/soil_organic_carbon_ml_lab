...
def observed_soil_dynamic_properties():
    ...
    if uploaded_file is not None:
        try:
            if 'soil_organic_carbon_predictions' in data.columns:
                data['soil_organic_carbon_stocks'] = data['soil_organic_carbon_predictions'] * data['depth_cm'] * data['bd_mean']
        ...
        features1 = [
            'land_cover', 'land_cover_class', 'land_use', 'land_use_class',
            'total_precipitation', 'min_temperature', 'mean_temperature', 'max_temperature',
            'dem', 'slope', 'aspect', 'hillshade', 'bd_mean', 'clay_mean', 'om_mean',
            'ph_mean', 'sand_mean', 'silt_mean', 'soil_organic_carbon',
            'soil_organic_carbon_stocks', 'label'
        ]
    features1.sort()
    ...
    filtered_data = data[(data["year"] >= min_year - 1) & (data["year"] <= max_year)]
    ...
    try:
        ...
        depth_filter = (filtered_data0['depth_cm'] == int(depth_c.split('-')[-1])) if soil_properties in terraincls else (filtered_data0['depth_cm'] == 5)
